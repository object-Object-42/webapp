...
async def transcribe(
    *, file: UploadFile = File(...), session: SessionDep
    ...
    try:
        ...
        content_data = ContentBase(
            doc_name=file.filename, content_text=transcription.text, url=None
        )

        db_content = create_content(
            session=session, content_data=content_data, org_id=42
        )
